...
# Problem 7: Server CPU Temperature Check (One-Sample T-Test, Right-Tailed)

## 📌 Problem Overview

A data center technician suspects a cooling fan failure because a rack server's mean CPU operating temperature under heavy load appears higher than the safe target baseline of **72.0°C**.

* **Baseline Mean ($\mu_0$):** $72.0\text{°C}$
* **Sample Size ($n$):** $22\text{ stress readings}$ (Degrees of Freedom, $df = 21$)
* **Sample Mean ($\bar{x}$):** $75.8\text{°C}$
* **Sample Standard Deviation ($s$):** $6.2\text{°C}$
...
Testing whether the temperature is *higher* than baseline makes this a **right-tailed test**.

* **Null Hypothesis ($H_0$):** $\mu \le 72.0$  
  *(The CPU is operating at or below safe temperature thresholds.)*
* **Alternative Hypothesis ($H_a$):** $\mu > 72.0$  
  *(The CPU operating temperature is significantly higher than safe thresholds.)*
...
#### 1. Calculate the Standard Error ($SE$)
$$SE = \frac{s}{\sqrt{n}} = \frac{6.2}{\sqrt{22}} \approx \frac{6.2}{4.6904} \approx 1.3218\text{°C}$$

#### 2. Calculate the $t$-Score
$$t = \frac{\bar{x} - \mu_0}{SE} = \frac{75.8 - 72.0}{1.3218} = \frac{3.8}{1.3218} \approx 2.87$$
...
#### Method A: Critical Value Approach
* For a right-tailed test at $\alpha = 0.01$ with $df = 21$:

  $$t_{\text{critical}} = 2.518$$

* **Decision Rule:** Reject $H_0$ if $t_{\text{calculated}} \ge 2.518$.
* Since $2.87 \ge 2.518$, $t_{\text{calculated}}$ falls into the **rejection region**.

#### Method B: $P$-value Approach
* $$P\text{-value} = P(t_{21} \ge 2.87) \approx 0.0046 \quad (0.46\%)$$
* Since $0.0046 \le 0.01$, the result is **statistically significant**.
...
* **Statistical Decision:** **Reject $H_0$** (the Null Hypothesis).
* **Contextual Conclusion:** There is sufficient evidence at the $1\%$ significance level to conclude that the server CPU is operating at abnormally high temperatures.
